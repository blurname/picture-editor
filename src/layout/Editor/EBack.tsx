...
import { backCellShader, backImageShader, backPureShader } from '../../filter/shader'
import { backCellUniform,  backPureUniform } from '../../filter/uniform'
import {BackgroundSpirit, BackNonImageSpirit, BeamSpirit} from '../../utils/gl-uitls'
...
type Props = {
	...
	onChangeInput: (desc: string,func?: (a:any,b:any) => void) => (e: ChangeEvent<HTMLInputElement>) => void
}
...
export function EBack(props: Props) {
  ...
  enum RGB {
	...
	const changeRGB = <T extends Partial<keyof typeof RGB>>(rgb: T,value:number) => {
	...
	const [curSpirit, setCurSpirit] = useState<BackNonImageSpirit>(spiritCanvas.spirits[0] as BackNonImageSpirit);
